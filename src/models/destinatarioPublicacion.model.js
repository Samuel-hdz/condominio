...
destinatarioPublicacionSchema.pre('save', function () {
    if (this.tipo_destino === 'calle' && !this.calle_torre_id) {
        throw new Error('Para destino tipo "calle" se requiere calle_torre_id');
    }

    if (this.tipo_destino === 'domicilio' && !this.domicilio_id) {
        throw new Error('Para destino tipo "domicilio" se requiere domicilio_id');
    }

    if (this.tipo_destino === 'todos' && (this.calle_torre_id || this.domicilio_id)) {
        throw new Error('Para destino "todos" no se deben especificar calle_torre_id ni domicilio_id');
    }
});
